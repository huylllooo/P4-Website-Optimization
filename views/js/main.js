...
// Avoids creating anytime updatePosition is called
var items = document.getElementsByClassName('mover');
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");
  // Caches length of pizza container
  var itemsLength = items.length;
  var scrollTop = document.body.scrollTop;
  // Avoids re-calculation of phase in animation loop
  var phase = [];
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin((scrollTop / 1250) + i));
  }
  for (var i = 0; i < itemsLength; i++) {
   /* Thanks to Safadurimo for fixing the problem using transform (pizza.html edited)
      https://discussions.udacity.com/t/replace-left-with-transform/20093 */
    items[i].style.transform = 'translateX('+parseInt(items[i].basicLeft + 100 * phase[i%5])+ 'px)';
  }

  // User Timing API to the rescue again.
  // Super easy to create custom metrics.
  // Turned off to boost performance
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Reasonable number of moving pizzas
  for (var i = 0; i < 40; i++) {
